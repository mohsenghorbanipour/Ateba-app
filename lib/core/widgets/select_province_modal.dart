...
class SelectProvinceModal extends StatefulWidget {
  ...
  @override
  State<SelectProvinceModal> createState() => _SelectProvinceModalState();
}

class _SelectProvinceModalState extends State<SelectProvinceModal> {
  @override
  void initState() {
    selectedProvince =
        Provider.of<EditProfileBloc>(context, listen: false).selectedProvince;
    super.initState();
  }

  ConfigItem? selectedProvince;
                ...
                if (context.select<EditProfileBloc, bool>(
                  ...
                  Selector<EditProfileBloc, List<ConfigItem>>(
                    selector: (context, bloc) =>
                        (bloc.profileConfigResponse?.provinces ?? [])
                            .where((e) => e.name?.contains(searchKey) ?? false)
                            .toList(),
                    builder: (context, provinces, child) => Expanded(
                      child: ListView.separated(
                        padding: const EdgeInsets.symmetric(vertical: 12),
